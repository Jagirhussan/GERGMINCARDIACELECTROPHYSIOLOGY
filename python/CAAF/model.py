...
class Heart(Base):
    ...
    def __init__(
        ...
        prob_def=0,
        prob_not_fire=0.05,
        filenameprefix='./figures/animated_simulation'
    ):
    ...
    def _run_sim(self, num_iters, parameters):
        ...
        for t in range(num_iters):
            state = update_grid(
                time_step=self.time_steps_elapsed, state=self.state,cell_refractory_period=self.cell_refractory_period, **parameters
            )
            #Calculate condution velocity
            for r in range(0, self.row_size):
                for c in range(1, self.col_size - 1):
                    if state[r,c] != self.state[r,c] and state[r,c]==1:
                        self.conduction_velocity[r,c,0] += (t - self.conduction_velocity[r,c,2])
                        self.conduction_velocity[r,c,1] += 1
                        self.conduction_velocity[r,c,2] = t

            self.state = state
            self.time_steps_elapsed += 1
            num_active_cells[t] = np.sum(self.state[:, 1:-1] == 1)
            #Store the time series
            self.state_timeseries = np.dstack((self.state_timeseries,self.state))
    ...
    def _run_sim_plot(self, num_iters, parameters):
        ...
        ani.save(f"{self.filenameprefix}.mp4")
        fig.savefig(f"{self.filenameprefix}.png",dpi=600)
        self.num_active_cells = np.r_[self.num_active_cells, num_active_cells]
        return None
...
def update_grid(
    ...
    for r in range(0, row_size):
        for c in range(1, col_size - 1):
            ...
            if state[r, c] > 1:
                new_state[r, c] = np.mod(state[r, c] + 1, cell_refractory_period[r,c] + 2)
