...
class Base(object):
    ...
    def __init__(
        ...
        self.cell_refractory_period = refractory_period*0.9 + 0.1*np.random.rand(row_size,col_size)
        self.driving_period = (
            driving_period if driving_period is not None else refractory_period * 2
        )
        self.conduction_velocity = np.zeros((row_size,col_size,3))
        ...
        # Place holder for storing state values
        self.state_timeseries = self.state
